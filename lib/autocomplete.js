...
var getKeywordSuggestions = function(prefix, currentResourceKey, dotNotation, settings) {
  ...
  // Build replacement prefix. This is used by autocomplete-plus to replace the prefix with actual suggestion.
  var replacementPrefix;
  if(dotNotationInfo.dotNotation && !settings.avoidDotNotation){
    // Suggest using dot notation format.
    var start = prefix.length - dotNotationInfo.keywordPrefix.length;
    replacementPrefix = prefix.substring(start, prefix.length);
  } else{
